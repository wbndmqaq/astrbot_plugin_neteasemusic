...
- **QQ 官方机器人适配**：合并消息规避被动回复额度、大文件/FLAC 自动跳过文件改发语音、纯文本兜底规避 markdown 报错
...
| `#ncm相关歌单 关键词\|id` | 包含该曲的歌单 | `#ncm相关歌单 晴天` |
...
| 指令 | 说明 |
| --- | --- |
| `#ncm登录` | 扫码登录（生成二维码，轮询自动写入 Cookie） |
| `#ncm状态` / `#ncms` | 查看登录状态 |
| `#ncm登出` | 登出并清除本地 Cookie |
...
**Q：卡片不显示图片/渲染失败？**
A：卡片依赖 AstrBot 的 `html_render` 远程渲染服务，确认 AstrBot 版本 ≥ 4.26 且渲染服务可用；可关闭 `renderListCard` 退回纯文本。
